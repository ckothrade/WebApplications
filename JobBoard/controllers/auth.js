...
const transporter = nodemailer.createTransport(sgTransport({
  auth: {
    api_key: '' // UPDATEHERE : sendmail api key
  }
})); 
  ...
  bcrypt.hash(password, 12)
    ...
    .then(result => {
      ...
      return transporter.sendMail({
        to: email,
        from: '', // UPDATEHERE : add verified sender email
        subject: 'New Account',
      ...
      .then(result => {
        ...
        transporter.sendMail({
          ...
          html: `
            ...
            <p>Click <a href="http://localhost:/reset/${token}">here</a> to reset your password...</p>
          ` // UPDATEHERE : add port number after colon
        });
      })
      .catch(err => console.log(err));
  });
